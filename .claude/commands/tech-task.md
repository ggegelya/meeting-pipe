---
description: Implement a TECH-* task from the Q2 backlog, one task per session, one commit
---

You're picking up a single backlog task: $ARGUMENTS

1. Read the task definition from `~/Downloads/meetingpipe-q2-backlog.md`. The task ID format is `TECH-<letter><number>` (e.g. `TECH-E5`). Search for the line starting with `**TECH-...` matching the ID. If you can't find it, stop and tell me.
2. Read the orientation docs the task touches:
   - [`CLAUDE.md`](CLAUDE.md) at the repo root for git workflow + verification + conventions worth knowing.
   - [`ARCHITECTURE.md`](ARCHITECTURE.md) for the subsystem map.
   - [`CONVENTIONS.md`](CONVENTIONS.md) for code patterns + the event log + sidecar schemas.
...
7. Mark the task `[DONE]` in `~/Downloads/meetingpipe-q2-backlog.md` (prefix the `**TECH-...` line — keep the trail, don't delete).
8. Commit on `main` (no branch) as `Georgy <g.gegelya@icloud.com>`:
   ```bash
   git -c user.name=Georgy -c user.email=g.gegelya@icloud.com commit -m "TECH-<ID>: <short summary>"
   ```
