...
- `summarization.user_label`: display name stamped on your own diarized speaker so "me vs them" reads as your name instead of `Speaker 1`. Picked structurally (the mic channel on a stereo recording, else the dominant speaker by talk time); blank (default) keeps generic labels. Surfaced as **Your name** in Preferences → Pipeline.
