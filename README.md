...
- `summarization.summary_language`: `"auto"` (default; matches transcript language) or an ISO 639-1 code to force a specific output language. Local backends used to drift on Ukrainian (the 7B rendered it as English, the 14B as Russian); the on-device path now reinforces the target and retries once when it still detects an en/uk/ru mismatch, so a Ukrainian meeting yields a Ukrainian summary (LOCAL7).
