...
| `meetingpipe://library` | Open the Library (`?scope=ask` / `?scope=digests` / `?scope=facts` / `?scope=people` / `?scope=time` to jump to a rail) |
...
3. **Grant macOS permissions** when prompted on first launch. On a fresh install a Welcome window walks you through the four permissions one at a time, each framed with what it is for and requested only when you click its row (so the system dialogs never stack unlabelled over the window). On every launch after onboarding is done, the daemon instead re-checks and fires any still-needed dialog in one ordered sequence within the first few seconds, rather than dribbling them out across the first recording. The four, in the order the onboarding step lists them:
   ...
   The **Preferences ▸ Permissions** tab shows the live status of all four with **Request** (when never determined) or **Open Settings** (when denied) buttons; the menu bar surfaces a warning row per missing permission that names what stops working (for example, "⚠ Screen Recording off: the call audio will not be recorded"), for mic, Screen Recording, or Accessibility. Recording is gated on Microphone: if it's missing, the daemon refuses to start the recording and routes you to the Permissions tab rather than producing a silent wav.
...
# Re-run only the finalize stage over an existing <stem>.json: speaker
# labels, roster match, and glossary, with no summarize and no publish.
# The tail of the Library's Re-transcribe action.
mp finalize <wav>
...
- **Rail** (under a `LIBRARY` header): `All meetings`, `Today`, `Last 7 days`, `Last 30 days`, `Needs you`, `NDA only`, `Untagged`, and under an `INSIGHTS` header `Facts`, `People`, `Meeting time`, `Ask`, `Digests`. `Needs you` carries an amber count badge and gathers the meetings that want an action: a failed run, a long-meeting bundle awaiting a paste, or a finished meeting whose publish failed or only partially landed. `Facts` is a cross-meeting projection rather than a list filter: it gathers every open action (with its owner, an aging label off the due date, and a circle to mark it done in place) and the decisions from meetings in the last 30 days, each row linking back to its source meeting. Marking an action done writes the resolved flag into that meeting's summary, so it survives a republish. `Facts` carries the same amber count badge `Needs you` does, showing how many commitments are overdue or due today, so an aging promise resurfaces from the rail without opening the view (AI7). Its rows are commitments, not occurrences: when a recurring meeting restates the same promise, the restatements group into one row tagged `restated Nx` (hover it to see which meetings), and marking that row done closes every instance at once, so resolving a standup commitment does not leave five clones open. The grouping runs `mp actions --cluster` on-device in the background; until it lands, and if it cannot run at all, the rows simply list ungrouped. `People` (DV3) is the second projection: one row per person you have named, with the meetings they were in, the open actions they own, and when you last saw them. Expand a row to get both lists; every meeting and every action links back to its source meeting. Its actions are the same commitments `Facts` shows, grouped the same way, so a promise someone restates every standup counts once here too and marking it done in `Facts` clears it from their row at the same moment. Membership follows the name, so a person shows up in a meeting when the transcript names them (whether the roster matched their voice or you named them in-app) or when the summary made them the owner of an action. **Rename…** on a row renames them everywhere: the roster entry, this rail, Preferences ▸ Pipeline ▸ People, and the past meetings that name them, which follow through the same reversible overlay a manual rename uses (your on-disk transcripts are untouched, and renaming back undoes it). Nobody appears here until you name a voice, so the rail is empty until then and says where to do it. `Meeting time` (AI8) is the third projection: where your hours went, one row per workflow over a range you pick (7 / 30 / 90 days, or all time), each row showing the total, the meeting count, the average length, and how much of the talking was yours. The talk share comes from the diarized transcript, counted in seconds of speech and weighted across the whole bucket, so a two-hour call does not count the same as a two-minute one; it needs your name set in Preferences → Pipeline → Your name, which is what lets the pipeline stamp your own voice in the transcript. A meeting where your voice was not identified (you never spoke, or diarization could not separate you) reads `not measured` rather than `you 0%`, and the footer says how many of the range's meetings that is, so a partial number never reads as a full one. Speech the diarizer credited to nobody is left out of the ratio rather than counted as somebody else. Read-only, fully on-device, and deliberately not a dashboard: no streaks, no targets, no trend lines. `Ask` (AI3) is the fourth projection: a question box over an engine-backed, cited answer across the whole library. It runs `mp ask` on-device (honouring the backend + egress clamp), shows a spinner while it works (answers are async per the AI2 spike, not live chat), and renders the answer with each verified citation linking back to its source meeting. Cmd+K Quick Find hands a question straight here, prefilled and running (UX25, described under the filter bar below), as does `meetingpipe://ask?q=…`. `Digests` (AI4) is the fifth projection: a read-only list of your weekly review digests. `mp digest` gathers your open actions and recent decisions across the library into a dated, on-device summary and writes it to a `digests` folder beside your recordings. Turn on a weekly schedule in Preferences ▸ Pipeline (it installs a per-user launch agent that runs `mp digest` at the day and time you pick, and removes it when you turn it off), or click **Generate now** in the Digests view; each digest renders with the standard summary layout, with reveal-in-Finder and move-to-Trash. Its action list is bounded to the review window and grouped under the workflow each commitment came from (AI10, described under `mp digest` below), and the reader links to `Facts` for the whole open set rather than dumping it into the read. Under a `SMART FOLDERS` header: your own saved searches (see below); the group only appears once you have saved one. Under a `WORKFLOWS` header: one row per workflow (colored dot + name + meeting count; the default workflow is labelled inline). A `+ New workflow` row opens an editor sheet. Workflows are filter scopes here, not a separate destination, and selecting one narrows the list AND surfaces the workflow inspector in the right column.
...
Select a row and the right pane fills with the per-meeting detail view: an editable title, the meeting's date, the workflow chip when present, duration, and shortcuts to open the published note in Notion or Obsidian (when those sidecars are on disk) or reveal the raw audio in Finder. When the backend that produced the summary is known, a quiet grey provenance line sits under the caption naming it - `Claude (cloud)`, `On-device (MLX)`, or `Apple Intelligence` (the model id rides in the tooltip); it is hidden for legacy, paste, or skipped meetings with no recorded backend. A `...` menu on the header collects the less-common actions (Rename, Edit summary, Reprocess, Re-summarize with… (Local / Anthropic), Re-transcribe…, Change workflow…, Corrections…, Republish, Open meta.json, Copy meeting ID, Delete…). **Change workflow…** re-routes a misrouted or manually-recorded meeting to a different workflow: it rewrites the meeting's context prompt, sinks, Notion database, and NDA posture, refreshes its chip and smart-folder membership, then offers to Regenerate (re-summarize with the new prompt, then republish) or Republish (re-send the existing summary to the new sinks). Neither runs unless you pick it. Moving into or out of an NDA workflow asks for explicit confirmation: NDA re-clamps everything to on-device + filesystem from now on, but it cannot un-publish anything already sent. Three tabs underneath:
...
Quick Find also answers questions, not just finds meetings (UX25). Every non-empty query carries an **Ask about your meetings** row that hands what you typed to the Library's `Ask` rail, prefilled and already running, so a question typed into Cmd+K reaches a cited answer without your having to know Ask is a separate surface. End the query with a question mark and the row leads the list, so `⌘K` → type → `↩` is the whole path; otherwise the row sits under the results, and `⌘↩` reaches it from anywhere in the panel. A query that matches no meeting is an Ask row rather than a dead end.

**Saving a search as a smart folder (UX24).** Once any filter is active, a folder-plus button appears next to **Clear**. It names the current view and pins it to the rail under `SMART FOLDERS`, where it behaves like a built-in scope: its own row, its own meeting count, its own list header. What gets saved is the whole view, not just the text: the rail scope you are in plus every chip. Saving from inside `Needs you` keeps `Needs you` as the folder's base; saving from a workflow scope folds that workflow into the folder's workflow chip, so deleting the workflow later widens the folder instead of breaking it. The sheet previews exactly what is about to be saved and warns if the name is already taken. Folders persist in `~/.config/meeting-pipe/saved_searches.json` and are there on the next launch. Right-click a folder for **Rename…**, **Update to current view** (re-points it at whatever you are looking at now, keeping its name and place), and **Delete**; hovering one shows its criteria. Selecting a folder still leaves the filter bar live, so you can narrow further within it, and saving again from there creates a child folder that inherits the parent's criteria.

Cmd+click (or Shift+click for a range) to select multiple rows. The detail pane swaps to a batch-actions panel that lists the selection and offers **Merge into one meeting…** (FEAT9, described below), **Re-transcribe with the current stack…** (described below), **Republish all** (runs `mp publish` per meeting, sequentially so the daemon's processing queue doesn't fan out; each meeting reaches every sink its workflow configures, not just Notion), **Export markdown…** (writes one `<stem>.md` bundle per meeting into a chosen folder), and **Move to Trash…** (per-row soft delete with one confirmation up front). A linear progress strip shows `<done> / <total>` while a batch is in flight.
...
**Re-transcribe with the current stack…** un-freezes an old transcript. A meeting's transcript is otherwise fixed at the quality it was captured at: a glossary term you added last month, a voice you named last week, or an improvement to the ASR and diarization models never reaches a meeting recorded before them, and Reprocess / Re-summarize only re-run the summary over the transcript already on disk. This transcribes each selected recording again from its audio and re-runs the finalize stage, so the glossary, your roster names, and the current models all apply. It is fully on-device: no engine call, nothing published, and each meeting keeps the summary it already has, so you are never left with a meeting that has none. When it finishes, a **Re-summarize all** button offers the follow-up that regenerates the summaries from the new transcripts and republishes them; that step is never automatic, because it is the one that can reach a paid engine and rewrite a published page. The same action sits in the detail `...` menu as **Re-transcribe…** for a single meeting.

Your hand edits are carried over, not preserved verbatim, and the difference matters. Speaker names and per-line text corrections are stored against a segment's position and its diarization cluster, and re-transcribing renumbers the segments and re-assigns the clusters, so keeping them as they are would land a name on a different voice. Instead each override is re-anchored by *time* onto whichever new segment or cluster covers the same stretch of audio. Anything the new transcript has no clear home for is dropped rather than guessed at, and the count is reported when the run finishes, so a lost edit is something you are told about rather than something you discover later. A text correction the new transcript already reads correctly (because the glossary now fixes it) is retired as no longer needed, which is also reported.
...
A/B the result against the base model with `mp dogfood --adapter <dir> <transcript>` (a local-only comparison, no cloud baseline) and grade the runs. Training reserves a held-out slice and records it beside the adapter, so `mp dogfood` refuses a meeting the adapter trained on and names one it did not; that keeps the scorecard a test of quality rather than of memory (`--allow-trained` overrides). If the adapter wins, opt it into the local backend by setting `summarization.local_adapter_path = "<dir>"` in `~/.config/meeting-pipe/config.toml`. The adapter then rides along with every request to the local model, and a path that does not exist stops the run with an error naming it rather than quietly falling back to the base model, so "I set it and nothing changed" is not a state you can end up in. Nothing is ever applied silently, and an honest negative result (the adapter is not worth adopting) is a fine outcome: the corpus stays a useful record either way.
...
No terminal needed for a quick triage: the menu bar's **Diagnostics…** item opens a read-only viewer over the same event logs, with the same since / category / action filters. And Preferences → Integrations → **Run doctor…** now runs the daemon's own self-check (Accessibility, Screen Recording, Microphone, per-app reachability, orphan scan, and search-index health) before `mp doctor`'s credential and network checks, so both halves are visible in one sheet.
...
mp actions --open --cluster                         # group a series' restatements
```
...
A recurring meeting restates the same commitments week after week, so without help the list accumulates near-duplicates and closing one leaves its clones open. `--cluster` (AI7) groups them: the task text of each open action is embedded on-device with the same multilingual model `mp ask` uses, and near-duplicates within one **workflow** merge into a single commitment, printed once with a `restated Nx` tag instead of once per occurrence. The workflow is the series key, so two different meeting series never merge, and an action on an untagged meeting is always its own commitment. Two guards keep it honest: two *named* owners never merge (only a matching or unattributed owner does), and the similarity floor is deliberately conservative, since leaving a duplicate on the list is far cheaper than silently closing something you never finished. Tune it with `--cluster-threshold 0.85` if your meetings need it. Everything stays on-device, and if the embedding model cannot run the actions simply list ungrouped rather than failing.
...
**The action list is a review, not a dump (AI10).** A digest used to roll up every open action in the library, which on a few hundred meetings is several hundred items and past the point where anyone reads it. It now shows the commitments this period is actually about: the ones raised in the window, plus anything already overdue or falling due inside it. What that leaves out is the dormant backlog, the undated action from a meeting months ago that nobody closed and nothing will ever make due. Age alone never drops an item: a promise from February that is 159 days past due is exactly what a weekly review is for, and it leads the list. What survives is grouped by workflow (by meeting title when the meeting is untagged), ranked most-pressing-first (overdue by how overdue, then due soonest, then undated, with a firmer commitment breaking a tie), and capped at 25 items with at most 5 per workflow, so one noisy series cannot crowd out the rest. The digest then appends an `Action scope` section saying exactly what it showed and what it did not: `Showing 10 of 14 open action(s) from the last 7 day(s), across 3 of 3 group(s)`, which workflows are trimmed, and the library-wide open total. The full list is one click away in the Library's Facts rail (the digest reader links straight to it) or in `mp actions --open` on the command line.
...
- Custom vocabulary (glossary): recurring proper nouns and client / product names that ASR mangles are normalized before summarize + embed ever see them, via a separate `~/.config/meeting-pipe/glossary.toml` (see [`glossary.example.toml`](./glossary.example.toml)). Matching is case-insensitive, whole-word, longest-match-first, and Unicode-aware (works for Cyrillic word boundaries). A `[terms]` table applies to every meeting; a `[workflow."<name>".terms]` table applies only to that workflow's meetings and wins on a clash. Applied at transcript finalize, so a new meeting picks it up automatically. An older meeting recorded before you added the term is reached by the Library's **Re-transcribe with the current stack…** (see the Library section), which is the one action that rewrites an existing transcript; Reprocess and Re-summarize only re-run the summary over the transcript already on disk. An optional `[fuzzy]` stage, off by default, also corrects near-miss spellings of longer names.
...
**The transcript I see right after stop has speaker labels but the next day they look different.** Nothing rewrites a transcript on its own, so something re-ran it. Three things can: a **Re-transcribe** from the Library (which is meant to change the labels, and reports what it did to your names and edits), a **Reprocess** on a failed row, or a manual `mp run-all <wav>`. Check `pipeline.log` for two run sections matching the file's timestamp, and `mp logs --category coordinator` for a `retranscribe_done` row.
...
**I closed the lid right after a meeting and the summary failed.** This one should now recover by itself. Each summarization request is bounded by a 120 s timeout that rides a clock which freezes while the Mac is asleep, so a request still in flight when you close the lid spends its whole budget across the brief wakes macOS takes overnight and reports a timeout on lid-open, having never had 120 s of awake time to answer in. The pipeline compares the wall clock against that frozen one, reads the gap as a sleep rather than a stalled connection, and re-issues the request once; `pipeline.log` carries a `timed out across a system sleep` warning naming both numbers whenever it does. A second timeout, or a genuine stall on a Mac that stayed awake, still fails the run and leaves a Failed row to Retry.
...
**I changed the local model (or the adapter) and summaries did not change.** Preferences → Integrations → **Run doctor…** (or `mp doctor`) now answers this directly under "local model server": it reports what the running server is actually serving next to what config asks for, and warns when the two differ. The cause is a warm server, which pins one model for as long as it runs. Both halves self-heal now: a server `mp` itself started is replaced on the next run, and the daemon's launch-time warm server (Preferences → Pipeline → "Preload local model at launch") restarts a couple of seconds after you save the change. If doctor still names a mismatch, quit and relaunch MeetingPipe, or kill the pid it prints. Until it clears, `<stem>.run.json` records the model that *served* rather than the one configured, so past summaries stay honestly attributed instead of silently crediting the new model.
