...
|---|---|---|---|---|
| TECH-SEC2 | Clamp sinks under global regulated mode | Security | P0 new | Global regulated mode forces the local LLM but does not strip the Notion sink, so the publish path still transmits meeting content; clamp sinks the way NDA does. |
...
| TECH-SEC3 | Process-wide egress firewall under regulated/NDA | Security | P1 new | Replace seven scattered flag checks with one httpx transport that hard-fails any non-loopback request when regulated or NDA is active. |
