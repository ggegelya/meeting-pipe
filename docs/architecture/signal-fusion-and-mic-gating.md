...
The device-scope sibling `kAudioDevicePropertyDeviceIsRunningSomewhere` was classified CORROBORATING here at design time, on the reasoning that it fires when the last client on the input device releases it. **That classification was wrong, and it is now REJECTED for meeting-end detection.** It assumed the daemon is an observer of the device; the daemon is a *client* of it. Our own capture holds an IOProc on the input device for the whole recording, so the flag can never read idle while a meeting is being recorded, and the "last client" never releases. Worse, `AVAudioEngine.inputNode` instantiates a `CADefaultDeviceAggregate` spanning both directions, so tapping the mic also pins the default **output** device RUNNING, which kills the "read the speaker side instead" rescue. Measured on 2026-07-14 by [`daemon/scripts/end6-device-idle-probe.swift`](../../daemon/scripts/end6-device-idle-probe.swift); see [`docs/spikes/end6-device-idle-corroborator.md`](../spikes/end6-device-idle-corroborator.md) for the full trace and the END6 close. The per-process read above stays the only unconfounded audio-layer end signal, because it asks about the client's process rather than the shared device. `kAudioProcessPropertyDevices` going from `[inputDeviceID]` to `[]` for the meeting process is CORROBORATING and adds an audit-log detail about which device was released. `kAudioObjectPropertyMute` on the input device tracks only hardware/Control-Center mute and is REJECTED for meeting-end detection.
