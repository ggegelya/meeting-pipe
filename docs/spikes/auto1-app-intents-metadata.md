...
> **Outcome (2026-07-20): built, discoverable, NOT invocable. Blocked on code signing, and the spike's real lesson is that it measured the wrong risk.** The owner ran the probe (`MECHANISM GO`), elected to build, and the build landed: `Automation/MeetingPipeAppIntents.swift` (six intents + an `AppShortcutsProvider`), `MeetingPipeAppIntentsTests`, and the extraction step in `scripts/install.sh`. The metadata half works exactly as this spike predicted: all six actions **list in Shortcuts** with their parameters. But **invoking one fails** with "Shortcuts couldn't communicate with the app": the app is never contacted, never crashes, and logs no `automation` event.
>
> Six candidate causes were each excluded by measurement, so do not re-run them: the intent types ARE in the shipped binary (AppIntents linked, conformance sections present, not dead-stripped); Launch Services DOES register the app; the failure is identical with `openAppWhenRun` true and false; identical whether launchd execs the binary or Launch Services opens it; identical with `LSUIElement` true and false; and the app never crashes. The one cause left standing is **code signing**: the bundle is `spctl`-rejected with `TeamIdentifier=not set`, uniquely so among third-party apps on the dogfood Mac (Ghostty / Obsidian / Notion all pass). Discovery reads a static file and needs no trust; the intent handoff is an XPC path that does. That also explains why `meetingpipe://` URLs keep working, and they are now **verified on real hardware** (`automation command {'verb': 'open_library'}`), which closes AUTO1's long-standing owner-owed routing check.
>
> So `install.sh` emits the metadata only under `MP_APP_INTENTS=1` (default off): shipping it would advertise six actions that every click fails on, which is worse than not offering them. The code is correct and one signed build away from working.
>
> **The methodological lesson.** This spike measured the metadata mechanism, declared `MECHANISM GO`, and called the build "bounded". It was right about the mechanism and wrong about the risk: the blocker was never the private extraction flags it worried about, it was a trust boundary the spike never probed, because the probe stopped one step short of the thing that actually mattered (invoking an intent, not just building its metadata). The "converge with DIST1" recommendation below turned out to be correct for a reason it did not anticipate. A probe that ends before the user-visible outcome can return GO on a leg that does not work.
