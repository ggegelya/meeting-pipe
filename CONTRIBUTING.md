...
- Backlog-task commits use `TECH-<ID>: <short summary>`. Other commits follow `fix(scope): ...`, `feat(scope): ...`, or `chore(scope): ...`. Match the style for the kind of change.
- The active backlog lives in [`docs/backlog/`](./docs/backlog/) (highest-numbered quarter file). Task IDs look like `TECH-E5`.
