...
| Worktree janitor gates | `sh scripts/test-prune-worktrees.sh` - mandatory after any edit to `scripts/prune-worktrees.sh`. It deletes directories, so every gate is pinned, including the 2026-07-24 regression where a live session's worktree was swept. Builds a scratch repo in a temp dir; touches nothing real. |
...
- Parallel sessions run one git worktree each, under `.claude/worktrees/`, branched from local `main` (`worktree.baseRef: head`). A worktree session still makes its one commit, then lands it itself: rebase onto `main`, then `git -C <main checkout> merge --ff-only`. `/tech-task`, `/file-task`, `/spike`, and `/audit` each carry the exact procedure as their second-to-last step. History stays linear, so no session ever leaves a merge bubble or a stranded branch. Leaving the main checkout on `main` is what keeps this working; uncommitted work there is fine, since a fast-forward refuses by itself rather than clobbering it.
- A session never deletes its own worktree. Removing the directory you are standing in works, and then every later tool call in that session dies on a missing cwd, so a follow-up question would find a broken session. Cleanup is a janitor instead: `scripts/prune-worktrees.sh`, run by a `SessionStart` hook and again by step 0 of the four commands, so each session clears its predecessors' leavings. `ExitWorktree` only ever applies to a worktree the session created itself; it is a no-op for one made by the new-conversation checkbox.
- The janitor's gates are all reasons to **keep**, and a worktree is removed only when every one of them clears: it sits under `.claude/worktrees/`, is not the caller's own, is on a branch fully merged into `main`, **has committed at least once**, has an idle `HEAD` and branch reflog, has no recent session transcript, and has a clean tree. "Merged and clean" alone does **not** mean finished, and reading it that way deleted a live session's worktree on 2026-07-24: a session in its first minutes is zero commits ahead of `main`, so it is trivially an ancestor, and its tree is clean only because it has not written anything yet. The reflog gate is what closes that hole. `scripts/test-prune-worktrees.sh` pins every gate, including that regression.
- What the janitor still cannot see: a session that landed its commit, stayed open past `MP_PRUNE_MIN_AGE_MIN` (default 60 minutes), and left no session transcript can still lose its directory. Its work is already on `main` by definition, so the cost is a dead cwd in an open session, not lost work. Set `MP_PRUNE_MIN_AGE_MIN=0` to sweep known-dead debris immediately; it relaxes only the age gates, never the reflog or clean-tree ones.
