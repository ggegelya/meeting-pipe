...
- A session never deletes its own worktree. Removing the directory you are standing in works, and then every later tool call in that session dies on a missing cwd, so a follow-up question would find a broken session. Cleanup is a janitor instead: `scripts/prune-worktrees.sh` removes worktrees whose branch is fully merged into `main` and whose tree is clean, plus their branches, skipping the caller's own. A `SessionStart` hook runs it, and step 0 of the four commands runs it again as a backstop, so each session clears its predecessors' leavings. In-flight work is never at risk, since an unlanded session has unmerged commits or a dirty tree or both. Note that `ExitWorktree` only ever applies to a worktree the session created itself; it is a no-op for one made by the new-conversation checkbox.
