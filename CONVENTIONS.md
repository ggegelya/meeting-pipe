...
**`<stem>.speaker_labels.json`** next to `<stem>.wav`: `{labels: {<raw label>: <name>}, segments: {<index>: <label>}}`, daemon-owned (Swift `SpeakerLabelStore`), read + written, **not** a Swift-to-Python contract (Python never reads it). The reversible overlay for in-app speaker naming (FEAT3-UNDO): rather than rewriting `<stem>.json` in place (the old one-way door), the daemon enrolls the voiceprint and records the assigned name here, resolved at display time in `TranscriptLoader.load`. `labels` name a whole diarization cluster; `segments` override a single segment (FEAT3-SEGMENT), winning over the cluster. Undo drops the entry, so the original diarization label in `<stem>.json` is always recoverable. Parallels `<stem>.transcript_corrections.json` (the text-correction overlay); an empty overlay deletes the sidecar.
