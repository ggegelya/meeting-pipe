...
final class RMSGateProbeTests: XCTestCase {
    ...
    // MARK: - Sustained-open (capture-first app-mute override)

    func test_sustained_open_fires_only_after_dwell() {
        let clock = TestClock()
        let probe = RMSGateProbe(
            closeThresholdDb: -55, openThresholdDb: -45,
            closeDwellMillis: 350, openDwellMillis: 80,
            sustainedOpenDwellMillis: 1200, clock: clock.read
        )
        var sustained: [Bool] = []
        probe.onSustainedOpenChange = { sustained.append($0) }

        // Open the gate (80 ms of voice).
        _ = probe.ingest(dBFS: -30)
        clock.advance(0.1)
        XCTAssertEqual(probe.ingest(dBFS: -30), .open)
        XCTAssertTrue(sustained.isEmpty, "open is not yet sustained")
        XCTAssertFalse(probe.sustainedOpen)

        // Hold open past the sustained dwell.
        clock.advance(1.2)
        _ = probe.ingest(dBFS: -30)
        XCTAssertEqual(sustained, [true])
        XCTAssertTrue(probe.sustainedOpen)
    }

    func test_sustained_open_clears_on_close() {
        let clock = TestClock()
        let probe = RMSGateProbe(sustainedOpenDwellMillis: 1200, clock: clock.read)
        var sustained: [Bool] = []
        probe.onSustainedOpenChange = { sustained.append($0) }

        _ = probe.ingest(dBFS: -30)
        clock.advance(0.1)
        _ = probe.ingest(dBFS: -30)            // open
        clock.advance(1.2)
        _ = probe.ingest(dBFS: -30)            // sustained
        XCTAssertEqual(sustained, [true])

        // Silence past the close dwell closes the gate and clears sustained.
        _ = probe.ingest(dBFS: -70)            // start close accumulation
        clock.advance(0.4)
        XCTAssertEqual(probe.ingest(dBFS: -70), .closed)
        XCTAssertEqual(sustained, [true, false])
        XCTAssertFalse(probe.sustainedOpen)
    }

    func test_brief_open_never_reaches_sustained() {
        let clock = TestClock()
        let probe = RMSGateProbe(sustainedOpenDwellMillis: 1200, clock: clock.read)
        var sustained: [Bool] = []
        probe.onSustainedOpenChange = { sustained.append($0) }
        ...
        clock.advance(0.4)
        XCTAssertEqual(probe.ingest(dBFS: -70), .closed)
        XCTAssertTrue(sustained.isEmpty, "closed well before the sustained dwell")
    }

    func test_reset_clears_sustained_state() {
        let clock = TestClock()
        let probe = RMSGateProbe(sustainedOpenDwellMillis: 1200, clock: clock.read)
        _ = probe.ingest(dBFS: -30)
        clock.advance(0.1)
