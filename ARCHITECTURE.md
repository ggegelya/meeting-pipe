...
- **Menu bar:** `StatusBarController.swift` - title, icons (outline/filled per `UISettings.menuBarIconStyle`), lock glyph for regulated mode, model-download progress, aggregate permission warning. The derivation behind it is `StatusBarModel.swift` (T3): a pure `derive(Inputs) -> Presentation` giving the title, icon, menu header, and the conditional rows in order, so the always-visible surface is table-tested without an `NSStatusItem`. The controller keeps the `NSMenuItem` construction. The **"Finish setup" checklist row** (UX22) is a second pure model, `SetupChecklist.swift`: given the same signals `mp doctor` reads (secrets, config, permissions, the local-model cache, unioned across workflows), it returns the still-unmet setup items, each deep-linked to its fix (Integrations, Permissions, or the model download); the row disappears when the list is empty. Inputs are gathered in `Coordinator.setupChecklistInputs()`.
