...
**Lockon** - the lifecycle and `MicGate` subsystems walk the meeting window's AX subtree once at recording start and cache the handles (Leave button, Mute button), so they observe the same window for the meeting's lifetime instead of re-walking. `MeetingAXWindowWatcher` picks up call-control windows that appear later, and defends against the recurring failure where the walk hands it a *stale* mute button (Teams' backgrounded hub / pre-join window) alongside the live one. Three mechanisms, each for a different shape of staleness, and the latter two mode-gated to capture-first so the regulated gate is never weakened: a **blind streak** (the live control became unreadable, so a latched `.muted` is cleared); a **VAD contradiction** (the read stays confidently `.muted` while the OS voice-activity detector reports sustained voice, so the read is stale and is discredited, MIC10 part 2); and a **cross-window disagreement** (two windows return conflicting mute buttons, so neither can be trusted and the read is treated as blind rather than letting the stale one win, MIC10 part 1). The walk returns at most one mute button per window, so a disagreement is always between windows, never within the live call.
