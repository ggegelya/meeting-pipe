...
| `~/.config/meeting-pipe/roster.json` | daemon writes; pipeline reads + writes | FEAT3-ROSTER named voices: each enrolled person with their embeddings and sample count, so a named voice is auto-named in later meetings. Managed in Preferences ▸ Pipeline ▸ People or via `mp roster list/rename/forget` |
