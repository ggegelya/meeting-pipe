...
- **Menu bar:** `StatusBarController.swift` - title, icons (outline/filled per `UISettings.menuBarIconStyle`), lock glyph for regulated mode, model-download progress, aggregate permission warning. The derivation behind it is `StatusBarModel.swift` (T3): a pure `derive(Inputs) -> Presentation` giving the title, icon, menu header, and the conditional rows in order, so the always-visible surface is table-tested without an `NSStatusItem`. The controller keeps the `NSMenuItem` construction.
