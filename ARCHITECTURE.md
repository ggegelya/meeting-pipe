...
**Search index** - `SearchIndex` (a `SQLite3` FTS5 wrapper) + `SearchIndexer` (keeps it in sync with `MeetingStore`), UX16. A rebuildable cache under `~/Library/Caches/MeetingPipe/search/` (ADR 0003: an index over files, not a database of record), built incrementally off the store's revision/signature deltas with the transcript read off the scan path. `MeetingFilterEngine.apply` and `QuickFindRanker.rank` take its candidate stems and union them with the in-memory corpus, so search never regresses when the index is still building. Uses the system `SQLite3` module, no package dependency.
