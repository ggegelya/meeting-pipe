...
- **Library window:** `LibraryWindow.swift` + `LibrarySidebar.swift` + `LibraryListView.swift` + `MeetingDetailView.swift` + three detail tabs (`SummaryTab`, `TranscriptTab`, `AudioTab`). `CorrectionsTab` is no longer a tab: the DSN2 IA pass demoted it to a sheet off the detail `...` menu, and dropped the Raw-files tab entirely (`RawFilesTab.swift` was kept on disk for a while afterwards and deleted in DOC9; sidecars are reached through `Open meta.json` and reveal-in-Finder). Reads `~/Documents/Meetings/raw/*.meta.json` via `MeetingStore.swift`. Filter / search via `MeetingFilter.swift`. The INSIGHTS rail group hosts the cross-library projection views: `FactsView` (DV1), `AskView` (AI3), and `DigestsView` (AI4, the weekly digests in the `digests` sibling). The SMART FOLDERS group (UX24) holds the user's saved searches: `SavedSearch.swift` (the pure value plus `capture` / `apply`), `SavedSearchStore.swift` (JSON persistence), and `SavedSearchViews.swift` (the criteria summary + the naming sheet). Filter-bar state lives on `LibraryRootView` alongside `scope`, because saving a folder needs both.
