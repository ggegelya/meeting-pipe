...
**Smart folders** - the left-rail scopes in the Library window. Under `LIBRARY`: All meetings, Today, Last 7 days, Last 30 days, Needs you (an amber-badged gather of meetings wanting an action), NDA only, Untagged. Under `INSIGHTS`: Facts, People, Ask, Digests, which are cross-library projections rather than list filters. Under `WORKFLOWS`: one scope per workflow. Powered by `LibraryScope` + `MeetingFilter`, in-memory. The free-text search box (both the filter bar and Cmd+K Quick Find) runs through a SQLite FTS5 index over transcripts + summaries (UX16, the shipped TECH-A3 upgrade); see *Search index* below.
