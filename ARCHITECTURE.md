...
- **Prompt panel:** `MeetingPromptWindow.swift` - the top-right "Record / Skip / Record (BYO)" panel that pops on detection. It also hosts the CAL2 "Last time" disclosure: `PrepCard.swift` is the pure projection (last meeting in this workflow, its opening summary points, the actions it left open) plus `PrepCardStore`, the off-main newest-first scan over `<stem>.meta.json` + `<stem>.summary.json`; `PrepCardView.swift` renders it as a stratum the panel grows to reveal, never a popup. The button collapses to zero width when the scan finds nothing, so a workflow's first meeting looks unchanged. `mp prep <workflow>` is the shell twin over the same two sidecars.
