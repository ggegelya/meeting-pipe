...
4. **Match the mac, then earn deviations.** Default to AppKit conventions: SF Pro at 13px, SF Symbols, sentence case for chrome (Title Case for menu commands), hairline borders not heavy shadows, fixed window sizes, system Dark Mode auto-follow, paper-warm canvas. When a custom pattern is right (the Library's smart-folder rail, the workflow tint on the HUD, the two-channel waveform), keep it grounded in native materials.
